...
## Implemented evidence extension (ShortsScope)

The new `measure` primitive adds forward/backward LK tracks with RANSAC ROI transforms, optional Tesseract text boxes/crops, supplied-font shape ranking and optional MediaPipe pose availability. Analysis can consume dense measurements while selecting fewer full frames. These are independently callable local tools, not implementations of TS-LLaVA or LLaVA-MLB internals. IG-VLM remains a precedent for the coarse grid; tool-directed refinement is an engineering adaptation.

A same-image/model before/after corrected one reference's fixed-camera implication using screen-left background evidence. Running/walking remained unresolved. OCR fixture geometry and supplied font ranking were validated, not universal text-style recovery. See [evidence CLI](EVIDENCE.md) and [benchmark scope/results](BENCHMARK.md).
